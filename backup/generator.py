...
def generate_formula(
...
) -> tuple[list[str], list[str], str, str, str, CoeffTriple]:
	...
	for index in range(1, alias_count + 1):
		...
		definitions.append(f"{name}(x) = {format_affine(intercept, slope)}")
	...
	multilinear_equation = f"{format_formal_polynomial(sum_formal(lhs_terms))} = {format_formal_polynomial(sum_formal(rhs_terms))}"
	quadratic_form = format_quadratic_form(add_quadratic(sum_quadratic(lhs_terms), scale_quadratic(sum_quadratic(rhs_terms), -1)))
	other_constants = collect_scalar_names(other_scalars, formula_scalars)
	return definitions, other_constants, equation, multilinear_equation, quadratic_form, total
...
def main() -> None:
	...
	definitions, other_constants, equation, multilinear_equation, quadratic_form, coeffs = generate_formula(
		seed=args.seed,
	...
	for constant_name in other_constants:
	...
	print(f"  {multilinear_equation}")
	print()
	print("Quadratic form:")
	print(f"  {quadratic_form}")
	print()
