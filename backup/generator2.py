...
def sample_symbol_assignments(rng: random.Random, symbols: list[str], *, exclude_zero: bool = False) -> dict[str, int]:
	assignments: dict[str, int] = {}
	for symbol in symbols:
		assignments[symbol] = sample_nonzero_integer(rng) if exclude_zero else sample_integer(rng)
	return assignments
...
def evaluate_correctness(bundle: FormulaBundle, seed: int | None) -> tuple[list[str], list[str], list[str]]:
	...
	base_symbols.extend(bundle.other_constants)
	seen: set[str] = set()
	...
	base_assignments = sample_symbol_assignments(rng, ordered_base_symbols, exclude_zero=True)
	x_value = sample_nonzero_integer(rng)
	...
	generated_lhs = evaluate_quadratic(bundle.lhs_quadratic, base_assignments, x_value)
	generated_rhs = evaluate_quadratic(bundle.rhs_quadratic, base_assignments, x_value)
